    ...
    * We also do stim data decoding which returns the stim data as an int16 of appropriate magnitude. Please
    ...
    def _parse_header(self):
        ...
        if not self.filename.suffix in [".rhd", ".rhs"]:
            raise ValueError(
                f"{self.filename} is not a valid Intan file. Expected .rhd or .rhs extension"
            )
        ...
        if self.filename.suffix == ".rhs":
            if self.filename.name == "info.rhs":
                if any(
                    (self.filename.parent / file).exists()
                    for file in stream_name_to_file_name_rhs.values()
                ):
                    self.file_format = "one-file-per-signal"
                    raw_file_paths_dict = create_one_file_per_signal_dict_rhs(
                        dirname=self.filename.parent
                    )
                else:
                    self.file_format = "one-file-per-channel"
                    raw_file_paths_dict = create_one_file_per_channel_dict_rhs(
                        dirname=self.filename.parent
                    )
            else:
        ...
        # attached to the actual binary file with data
            ...
            if self.filename.name == "info.rhd":
                ...
                if any(
                    (self.filename.parent / file).exists()
                    for file in stream_name_to_file_name_rhd.values()
                ):
                    self.file_format = "one-file-per-signal"
                ...
                # then there is one-file-per-channel where each channel in a neo stream is in its own .dat file
                    ...
                    self.file_format = "one-file-per-channel"
                    raw_file_paths_dict = create_one_file_per_channel_dict_rhd(
                        dirname=self.filename.parent
                    )
        ...
        if self.file_format == "header-attached":
            self._raw_data = np.memmap(
                self.filename, dtype=memmap_data_dtype, mode="r", offset=header_size
            )
        ...
        # for one-file-per-channel we have one memory map / channel stored as a list / neo stream
            ...
            for stream_name, stream_dtype in memmap_data_dtype.items():
                ...
                channel_memmap_list = [
                    np.memmap(fp, dtype=stream_dtype, mode="r")
                    for fp in channel_file_paths
                ]
        ...
        # signal stream and then we just take the longest one
            ...
            self._max_sigs_length = max(
                [raw_data.size for raw_data in self._raw_data.values()]
            )
        ...
        # are in a list we just take the first channel in each list of channels
            ...
            self._max_sigs_length = max(
                [raw_data[0].size for raw_data in self._raw_data.values()]
            )
        ...
        for signal_annotation in seg_annotations["signals"]:
            ...
            signal_annotation["intan_version"] = (
                f"{self._global_info['major_version']}."
                f"{self._global_info['minor_version']}"
            )
            global_keys_to_skip = [
                "major_version",
                "minor_version",
            ...
            global_keys_to_annotate = set(self._global_info.keys()) - set(
                global_keys_to_skip
            )
            ...
            reference_channel = (
                "hardware" if reference_channel == "n/a" else reference_channel
            )
            ...
            channel_info = next(
                (
                    info
                    for info in self._ordered_channel_info
            ...
            channel_keys_to_annotate = set(channel_info.keys()) - set(
                channel_keys_to_skip
            )
            ...
            for channel_id in channel_ids:
                matching_info = next(
                    info
                    for info in self._ordered_channel_info
                    if info["native_channel_name"] == channel_id
                )
    ...
    def _get_analogsignal_chunk_header_attached(
        self, i_start, i_stop, stream_index, channel_indexes
    ):
        ...
        if not stream_is_digital:
            ...
            for chunk_index, channel_id in enumerate(channel_ids):
                ...
                if multiple_samples_per_block:
                    sigs_chunk[:, chunk_index] = data_chan[
            ...
            if stream_is_stim:
                sigs_chunk = self._decode_current_from_stim_data(
                    sigs_chunk, 0, sigs_chunk.shape[0]
                )
            ...
            sigs_chunk = self._demultiplex_digital_data(
                digital_raw_data, channel_ids, i_start, i_stop
            )
        return sigs_chunk

    def _get_analogsignal_chunk_one_file_per_channel(
        self, i_start, i_stop, stream_index, channel_indexes
    ):
    ...
    def _get_analogsignal_chunk_one_file_per_signal(
        self, i_start, i_stop, stream_index, channel_indexes
    ):
        ...
        if stream_is_digital:
            ...
            output = self._demultiplex_digital_data(
                raw_data, channel_ids, i_start, i_stop
            )
        elif stream_is_stim:
    ...
    def _decode_current_from_stim_data(self, raw_stim_data, i_start, i_stop):
        ...
        sign_bit = np.bitwise_and(
            np.right_shift(data, 8), 0x01
        )  # Shift right by 8 bits to get the sign bit
...
def read_rhs(filename, file_format: str):
    ...
    with open(filename, mode="rb") as f:
        ...
        names_to_count = [
            name for name in stream_names if name not in special_cases_for_counting
        ]
        channel_number_dict = {
            name: len(stream_name_to_channel_info_list[name]) for name in names_to_count
        }

        # Each DC amplifier channel has a corresponding RHS2000 amplifier channel
        channel_number_dict["DC Amplifier channel"] = channel_number_dict[
        ...
        if file_format == "one-file-per-channel":
            ...
            channel_number_dict["Stim channel"] = len(
                raw_file_paths_dict["Stim channel"]
            )
            # Moreover, even if the amplifier channels are in the header their files are dropped
            channel_number_dict["RHS2000 amplifier channel"] = len(
                raw_file_paths_dict["RHS2000 amplifier channel"]
            )
        else:
            channel_number_dict["Stim channel"] = channel_number_dict[
                "RHS2000 amplifier channel"
            ]
    ...
    if bool(global_info["dc_amplifier_data_saved"]):
        ...
        for chan_info in stream_name_to_channel_info_list["RHS2000 amplifier channel"]:
            ...
            chan_info_dc["custom_channel_name"] = (
                chan_info_dc["custom_channel_name"] + "_DC"
            )
    ...
    for chan_info in stream_name_to_channel_info_list["RHS2000 amplifier channel"]:
        ...
        if file_format == "one-file-per-channel":
            ...
            stim_file_exists = any(
                [
                    amplifier_native_name in stim_file.stem
                    for stim_file in stim_file_paths
                ]
            )
        ...
        chan_info_stim["custom_channel_name"] = (
            chan_info_stim["custom_channel_name"] + "_STIM"
        )
        ...
        chan_info_stim["dtype"] = (
            "int16"  # this change is due to bit decoding see note below
        )
...
def read_rhd(filename, file_format: str):
    ...
    with open(filename, mode="rb") as f:
        ...
        version = Version(
            f"{global_info['major_version']}.{global_info['minor_version']}"
        )
        ...
        for g in range(global_info["nb_signal_group"]):
            ...
            channel_number_dict = {
                name: len(stream_name_to_channel_info_list[name])
    ...
    for chan_info in stream_name_to_channel_info_list[
...
def create_one_file_per_channel_dict_rhs(
    ...
    raw_file_paths_dict["timestamp"] = [Path(dirname / "time.dat")]
    raw_file_paths_dict["DC Amplifier channel"] = [
        file for file in files if "dc-" in file.name
    ]
    ...
    raw_file_paths_dict["Stim channel"] = [
        file for file in files if "stim-" in file.name
    ]
