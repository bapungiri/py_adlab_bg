    ...
    if signal_group_num_channels > 0 and signal_group_enabled > 0:
        for _ in range(0, signal_group_num_channels):
            add_channel_information(
                header, fid, signal_group_name, signal_group_prefix, signal_group
            )


def add_channel_information(
    header, fid, signal_group_name, signal_group_prefix, signal_group
):
    """Reads a new channel's information from fid and appends it to 'header'
    dict.
    """
    (new_channel, new_trigger_channel, channel_enabled, signal_type) = read_new_channel(
        fid, signal_group_name, signal_group_prefix, signal_group
    )
    append_new_channel(
        header, new_channel, new_trigger_channel, channel_enabled, signal_type
    )


def read_new_channel(fid, signal_group_name, signal_group_prefix, signal_group):
...
def print_header_summary(header):
    ...
    if header["dc_amplifier_data_saved"]:
        print(
            "Found {} DC amplifier channel{}.".format(
...
def get_bytes_per_data_block(header):
    ...
    bytes_per_block = bytes_per_signal_type(num_samples_per_data_block, 1, 4)

    # Amplifier data: Add 2 bytes per sample per enabled amplifier channel.
    bytes_per_block += bytes_per_signal_type(
        num_samples_per_data_block, header["num_amplifier_channels"], 2
    )
    ...
    bytes_per_block += bytes_per_signal_type(
        num_samples_per_data_block, header["num_board_adc_channels"], 2
    )

    # Analog outputs: Add 2 bytes per sample per enabled analog output channel.
    bytes_per_block += bytes_per_signal_type(
        num_samples_per_data_block, header["num_board_dac_channels"], 2
    )
...
def read_timestamps(fid, data, index, num_samples):
    ...
    data["t"][start:end] = np.array(
        struct.unpack(format_expression, fid.read(read_length))
    )
...
def read_digital_signal_type(fid, dest, start, num_samples, num_channels):
    ...
    dest[start:end] = np.array(
        struct.unpack("<" + "H" * num_samples, fid.read(2 * num_samples))
    )
...
def print_record_time_summary(num_amp_samples, sample_rate, data_present):
    ...
    if data_present:
        print(
            "File contains {:0.3f} seconds of data.  "
            "Amplifiers were sampled at {:0.2f} kS/s.".format(
                record_time, sample_rate / 1000
            )
        )
    else:
        print(
            "Header file contains no data.  "
            "Amplifiers were sampled at {:0.2f} kS/s.".format(sample_rate / 1000)
        )
...
def initialize_memory(header, num_samples):
    ...
    # digital input.)
    data["board_dig_in_data"] = np.zeros(
        [header["num_board_dig_in_channels"], num_samples], dtype=np.bool_
    )
    data["board_dig_in_raw"] = np.zeros(num_samples, dtype=np.uint)
    ...
    data["board_dig_out_data"] = np.zeros(
        [header["num_board_dig_out_channels"], num_samples], dtype=np.bool_
    )
    data["board_dig_out_raw"] = np.zeros(num_samples, dtype=np.uint)
...
def scale_timestamps(header, data):
    ...
    if num_gaps == 0:
        ...
        print(
            "Warning: {0} gaps in timestamp data found.  "
            "Time scale will not be uniform!".format(num_gaps)
        )
...
def extract_stim_data(data):
    ...
    data["compliance_limit_data"] = np.bitwise_and(data["stim_data_raw"], 32768) >= 1

    # Interpret 2^14 bit (charge recovery) as True or False.
    data["charge_recovery_data"] = np.bitwise_and(data["stim_data_raw"], 16384) >= 1

    # Interpret 2^13 bit (amp settle) as True or False.
    data["amp_settle_data"] = np.bitwise_and(data["stim_data_raw"], 8192) >= 1

    # Interpret 2^8 bit (stim polarity) as +1 for 0_bit or -1 for 1_bit.
    data["stim_polarity"] = 1 - (2 * (np.bitwise_and(data["stim_data_raw"], 256) >> 8))
...
def apply_notch_filter(header, data):
    ...
    if header["notch_filter_frequency"] == 0 or header["version"]["major"] >= 3:
        return
    ...
    for i in range(header["num_amplifier_channels"]):
        ...
        percent_done = print_progress(
            i, header["num_amplifier_channels"], print_step, percent_done
        )
